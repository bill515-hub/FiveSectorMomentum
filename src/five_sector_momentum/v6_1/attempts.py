...
def start(path: Path, scenario_id: str, sequence: int, output_dir: Path, registry_sha256: str, cap: int = 16) -> int:
    prior = rows(path)
    if len(prior) >= cap:
        raise RuntimeError("V6_1_ATTEMPT_CAP_EXCEEDED")
    attempt = len(prior) + 1
    record = [attempt, scenario_id, sequence, "STARTED", datetime.now().isoformat(), "", str(output_dir.resolve()), registry_sha256, ""]
...
def finish(path: Path, attempt: int, status: str, message_code: str = "") -> None:
    ...
    if not current or int(current[-1]["attempt"]) != attempt:
        raise RuntimeError("ATTEMPT_LEDGER_APPEND_ORDER_VIOLATION")
